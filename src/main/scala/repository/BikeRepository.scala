...
class BikeRepository(transactor: Transactor[IO]) {
  ...
  def findAllBikes: Stream[IO, Bike] = {
    ...
    sql"select model,brand,year, price,size, id from bikes"
      .query[Bike]
      .stream
      .transact(transactor)
  }
  ...
  def findBikeByID(id: Int): IO[Either[BikeNotFoundError, Bike]] = {
    val query =
      sql"select model,brand,year, price,size, id from bikes where id=$id"
        .query[Bike]
  ...
  def addBike(bike: Bike): IO[Either[BikeNotFoundError, Bike]] = {
    ...
    val query =
      sql"insert into bikes (model,brand,year,price,size) values (${bike.model},${bike.brand},${bike.year},${bike.price},${bike.size})"
    query.update.withUniqueGeneratedKeys[Int]("id").transact(transactor).map {
      id =>
        if (id > 0) {
          Right(
            new Bike(
  ...
  def deleteBike(id: Int): IO[Either[BikeNotFoundError, Int]] = {
    val query =
      sql"DELETE FROM bikes WHERE id = $id".update.run.transact(transactor)
    query.map { numberOfRows =>
      if (numberOfRows == 1) {
        ...
        Left(
          BikeNotFoundError(
            s"Bike with  id=${id} wasn't found- cannot be deleted."
          )
        )
      }
    }
  ...
  def updateBike(id: Int, bike: Bike): IO[Either[BikeNotFoundError, Bike]] = {
    val query =
      sql"UPDATE bikes SET model = ${bike.model}, brand = ${bike.brand}, year = ${bike.year}, price = ${bike.price}, size =${bike.size} WHERE id = $id"
