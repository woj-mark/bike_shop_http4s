...
//
 ...
 def findBikeByID(id:Int) : IO[Option[Bike]] ={
  val query = sql"select model,brand,year, price,size, id from bikes where id=$id".query[Bike]
  val action = query.option //Not sure whether the query would return a bike for given ID
  action.transact(xa)
 }
...
val bikeNameStream = sql"select model from bikes".query[String].stream.compile.toList.transact(xa)

//update
def updatBike(model: String, brand : String, year: Int, price : Int, size: String, id: Int) : IO[Int]= {
  //Returns the number of rows affected
  val query = sql"insert into bikes (id,model,brand,year,price,size) values ($id,$model,$brand,$year,$price,$size)"
 ...
 override def run(args: List[String]) : IO[ExitCode] = 
  updatBikeAutoGenerated.debug.as(ExitCode.Success)
